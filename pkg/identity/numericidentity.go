...
const (
	...
	// WellKnownReservedIdentity represents the minimal numeric identity that
	// represent well known identities.
	WellKnownReservedIdentity = NumericIdentity(100)
...
func initWellKnownIdentities() {
	...
	AddWellKnownIdentities(wellKnown)
}
...
var (
	...
	// ErrNotWellKnownIdentity is an error returned for an identity that is not
	// considered well known.
	ErrNotWellKnownIdentity = errors.New("not a well known identity")
)
...
// IsWellKnownIdentity returns true if the given NumericIdentity belongs
// to the well known identities.
func IsWellKnownIdentity(id NumericIdentity) bool {
	return id.Uint32() >= WellKnownReservedIdentity.Uint32() &&
		id.Uint32() < UserReservedNumericIdentity.Uint32()
}
